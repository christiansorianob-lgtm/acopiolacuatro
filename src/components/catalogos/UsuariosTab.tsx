...
export function UsuariosTab() {
  ...
  const handleOpenModal = (item?: Usuario) => {
    if (item) {
      ...
      setRol(item.rol);
      setPin(""); // Nunca mostramos el PIN actual
    } else {
      setEditingItem(null);
      setCedula("");
      setNombre("");
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!editingItem && !pin.trim()) return; // PIN es obligatorio al crear

    setSaving(true);
    let res;
    
    if (editingItem) {
      res = await updateUsuario(editingItem.id, { 
        cedula: cedula.trim(), 
        nombre: nombre.trim(),
        rol,
        ...(pin.trim() ? { pin: pin.trim() } : {}) // Solo actualizar PIN si se escribió algo
      });
    } else {
  ...
  const handleToggleActive = async (id: number, currentStatus: boolean) => {
    const res = await updateUsuario(id, { activo: !currentStatus });
    if (res?.success) {
      loadItems();
    }
  };
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-semibold text-white">Catálogo de Usuarios</h2>
          <p className="text-slate-400 text-sm">Gestiona los accesos al sistema.</p>
        </div>
        <button
                    ...
                    <tr key={item.id} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          item.rol === "ADMINISTRADOR" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-blue-500/10 text-blue-400 border border-blue-500/20"
                        }`}>
                          {item.rol}
                        </span>
                      </td>
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Cédula / Usuario</label>
            <input
              type="text"
              value={cedula}
          ...
          <div>
            ...
            <select
              ...
              disabled={saving}
            >
              <option value="OPERADOR">OPERADOR (Solo báscula)</option>
              <option value="ADMINISTRADOR">ADMINISTRADOR (Acceso total)</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">
              PIN de Acceso {editingItem && "(Dejar en blanco para no cambiar)"}
            </label>
            <input
              type="password"
              value={pin}
              onChange={(e) => setPin(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-white font-mono focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors"
              placeholder="Ej. 1234"
