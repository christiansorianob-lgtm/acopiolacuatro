...
export default async function ImprimirTiquetePage({ params }: { params: { id: string } }) {
  ...
  const formatearFecha = (fechaStr: Date | null) => {
        ...
        <div className="bg-slate-50 rounded-xl p-6 border-2 border-slate-200 mb-8 print:bg-white print:border-slate-300">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-2xl font-mono font-bold text-slate-700">{tiquete.pesoEntrada} Kg</p>
        ...
        <div className="mt-16 grid grid-cols-2 gap-8">
          <div className="text-center">
            <div className="border-t-2 border-slate-300 w-4/5 mx-auto pt-2">
              ...
              <p className="text-[10px] text-slate-400 uppercase mt-1">{tiquete.usuarioSalida?.nombre || tiquete.usuarioEntrada.nombre}</p>
            </div>
          </div>
          <div className="text-center">
            <div className="border-t-2 border-slate-300 w-4/5 mx-auto pt-2">
              <p className="text-xs font-bold uppercase text-slate-500">Firma Conductor</p>
              <p className="text-[10px] text-slate-400 uppercase mt-1">C.C. {tiquete.conductorNombre.split(" ").slice(-1)}</p> {/* Si guardamos cedula en el nombre */}
            </div>
          </div>
        </div>
